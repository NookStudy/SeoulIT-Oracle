...
주의사항
    1. start with에 minvalue보다 작은값을 지정할 수 없다. 즉 start with값은
    minvalue와 같거나 커야한다.
...
create sequence seq_serial_num
    ...
    cycle               -- 최대값 도달 시 시작값부터 재시작할지 여부 : YES
...
-- 생성한 인덱스 확인하기
/*
    데이터 사전에서 확인하면 PK 혹은 unique로 지정된 컬럼은 자동으로 
    인덱스가 생성되므로 이미 생성된 인덱스도 같이 확인된다.
*/
