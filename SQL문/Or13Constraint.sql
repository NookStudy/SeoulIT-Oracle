...
형식1] 인라인방식 : 컬럼 생성 시 우측에 제약조건을 기술한다.
    ...
    * 대괄호 부분은 생략 가능하고, 생략 시 제약명을 시스템이
    자동으로 부여한다.
*/
...
제약조건 및 테이블 목록 확인하기
    ...
    user_cons_columns : 테이블에 지정된 제약조건과 컬럼명의 간략한
        정보를 저장한다.
        user_constraint : 테이블에 지정된 제약조건의 보다 상세한 정보를
    저장한다.
...
-- 레코드 입력
insert into tb_primary1 (idx, user_id, use_name)
...
update tb_primary1 set idx = 2 where use_name = '유산슬';
/*
...
형식3] 테이블을 생성한 후 alter문으로 제약조건 추가
    alter table 데이블명 add [constraint 제약명] primary key (컬럼명);
*/
...
Foreign key : 외래키, 참조키
    ...
    - 만약 테이블간에 외래키가 설정되어 있다면 자식 테이블에 참조값이
...
-- 오류 발생. 부모테이블에는 gildong 이라는 아이디가 없음.
...
/*
    ...
    P : Primary key
    R : Reference integrity 즉 Foreign key 를 뜻한다.
    C : Check 혹은 Not Null
    U : Unique
*/
...
-- 부모테이블의 레코드는 삭제된다.
select * from tb_primary5;
-- 자식 테이블의 레코드는 남아있다. 단, 참조컬럼이 null로 변경된다.
select * from tb_foreign5;
...
create table tb_not_null (
    ...
    m_pw varchar2(30) null,     -- null 허용.(일반적으로 이렇게 사용 안 함)
...
/*
    default 값을 입력하려면 insert 문에서 컬럼 자체를 제외시키거나
