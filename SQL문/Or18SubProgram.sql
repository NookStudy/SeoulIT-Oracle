...
/*
서브프로그램(Sub Program)
    - PL/SQL 에서는 프로시저와 함수라는 두자리 유형의 서브프로그램이 있다.
    ...
    호출하는 경우는 거의 없다.
    - 프로시저는 외부 프로그램에서 호출하기 위해 생성한다. 따라서 Java, JSP
...
-- 프로시저의 실행은 호스트환경에서 excute명령을 부여한다.
...
*/ 
-- 프로시저 생성시 in파라미터를 설정한다. 사원테이블의 사원명 컬럼을 참조하는
-- 참조변수로 생성한다.
...
begin
    /*
    in파라미터로 전달된 사원명을 조건으로 급여를 구한 후 변수에 할당한다.
...
is
    ...
    필요하지 않아 is절은 비워둔다. 이와같이 변수 선언은 생략할 수 있다.
    */
begin
    /*
...
/
-- 호스트 환경에서 바인드변수를 선언한다. var 또는 variable둘다 사용할 수 있다.
...
create table zxopy_employees
    as
        select * from employees;
-- 복사되었는지 확인한다.
select * from zxopy_employees;

-- in파라미터는 사원번호와 급여를 전달한다. out파라미터는 적용된 행의 갯수를
-- 반환받는다.
...
is
    -- 추가적인 변수선언이 필요 없으므로 생략한다.
begin
    -- 실제 업데이트를 처리하는 쿼리문으로 in파라미터를 통해 값을 설정한다.
    update zxopy_employees
        set salary = p_salary
...
select first_name, salary from zxopy_employees where employee_id = 100;
-- 프로시저 실행. 바인드 변수에는 반드시 :을 붙여야한다.
execute pcd_update_salary(100, 30000, :r_Count);
-- update가 적용된 행의 갯수 확인
print r_count;
-- 업데이트 된 내용을 확인한다. 결과 : Steven 30000
select first_name, salary from zxopy_employees where employee_id = 100;
...
2. 함수
    ...
    정의한 것이다.
    - 함수는 in파라미터만 사용할 수 있고, 반드시 반환될 값의 자료형을 
    명시해야 한다.
...
/*
시나리오] 
2개의 정수를 전달받아서 두 정수사이의 모든수를 더해서 결과를 반환하는 
함수를 정의하시오.
    실행예) 2, 7 -> 2+3+4+5+6+7 = ??
*/
...
-- 함수는 in파라미터만 있으므로 in은 생략하는 것이 좋다.
...
begin
    -- 방법1
    -- 쿼리에서 실행된 결과를 into를 통해 변수의 저장한다.
...
create or replace function func_depatName (
    ...
    is
        ...
        return_dept_name departments.department_name%type;
begin
    -- 쿼리문을 실행한 후 결과값을 변수에 저장한다.
    select 
        department_name into return_dept_name
    from employees inner join departments using(department_id)
    where first_name = param_name;
    
    -- 결과 반환
    return return_dept_name;
end;
/
...
create trigger trig_dept_backup
    ...
    for each row
    /*
        행 단위 트리거 정의한다. 즉 하나의 행이 변화할 때마다 트리거가
        수행된다. 만약 문장(테이블)단위 트리거로 정의하고 싶다면 해당
        문장을 제거하면 된다. 이때는 쿼리를 한번 실행할 때 트리거도 단
        한번만 실행된다.
    */
...
delete from trigger_dept_original where department_id=103;
...
-- 원본테이블에서 5개의 레코드가 업데이트 되므로, 백업테이블에도 5개가
-- 입력된다. 행단위 트리거는 적용된 행의 갯수만큼 실행된다.
select * from trigger_dept_original;
select * from trigger_dept_backup;

/*
생성 2 : 오리지널 테이블에 업데이트 이후 테이블(문장) 단위로 발생되는
    트리거 생성
*/
create or replace trigger trigger_update_test2
    ...
    -- for each row 
    /*
    오리지널 테이블에 레코드를 업데이트 한 이후 테이블단위로 트리거가
    실행되므로 적용된 갯수의 상관없이 무조건 한번만 트리거가 실행된다.
    */
begin
    /*
    업데이트 이벤트가 감지되면 backup테이블에 레코드를 입력한다.
...
update trigger_dept_original set department_name = '5개업데이트2'
    where department_id>=10 and department_id<=50;
-- 5개의 레코드를 했지만, 테이블단위 트리거이므로 1개만 입력된다.
select * from trigger_dept_original;
select * from trigger_dept_backup;
