...
서브쿼리 
설명 : 서브쿼리 안에 또다른 쿼리문이 들어가는 형태의 select문
    where구에 select문을 사용하면 숴브쿼리 라고 한다.
****************/
-- HR 계정
...
*/
/*
시나리오] 사원테이블에서 전체사원의 평균급여보다 낮은급여를 받는 사원들을
    추출하여 출력하시오.
    출력항목 : 사원번호, 이름, 이메일, 연락처, 급여
*/
-- 1. 평균 급여 구하기 : 6462
select avg(salary) from employees;
--2. 해당 쿼리문은 문맥상 맞는듯 하지만 그룹함수를 단일행에 적용한
-- 잘못된 쿼리문이다. 오류가 발생한다.
select * from employees where salary<avg(salary);
-- 3. 앞에서 구한 평균급여를 조건으로 select문을 작성하시오.
...
/*
퀴즈] 전체 사원중 급여가 가장적은 사원의 이름과 급여를 출력하는
    서브쿼리문을 작성하시오.
    출력항목 : 이름1, 이름2, 이메일, 급여
*/
-- 1단계 : 최소급여를 확인한다.
...
/*
복수행 연산자 : any(어떤것이라도)의 개념은 or와 비슷하다.
    메인쿼리의 비교조건이 서브쿼리의 검색결과와 하나이상 일치하면
    참이되는 연산자. 즉, 둘중 하나만 만족하면 해당 레코드를 인출한다.
*/
/*
...
-- 3단계 : 둘중 하나만 만족하면 되므로 복수행 연산자 any를 이용해서 서브쿼리를
...
select first_name, salary from employees 
    ...
    select salary from employees where department_id = 20);
/*
    6000이상 이고, 동시에 13000보다 커야하므로 결과적으로 13000이상인
    레코드만 인출하게 된다. 결과 : 5명
*/
...
from 절에는 테이블이 들어와야 하는데, 아래의 서브쿼리에서는 사원테이블의
