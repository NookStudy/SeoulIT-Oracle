...
/*
1] inner Join(내부조인)
    - 가장 많이 사용되는 join문으로 테이블간에 연결조건을 모두 만족하는
    레코드를 검색할 때 사용한다.
...
/*
    ...
    인출된다. 즉, inner join은 조인한 테이블에서 양쪽 모두 만족하는 레코드만
    가져오게 된다.
*/
...
-- 2. 지역일련번호를 통해 부서 테이블의 레코드 확인하기 -> 21개의 부서 확인
...
*/
/*
시나리오] 사원 테이블과 부서테이블을 조인하여 각 직원이 어떤 부서에서
...
2] outer join (외부조인)
    ...
    않아도 기준이되는 테이블에서 레코드를 인출하는 join방식 이다.
...
/*
3] self join(셀프조인)
    셀프조인은 하나의 데이블에 있는 컬럼끼리 연결해야 하는 경우 사용한다.
...
매니저도 사원이기 때문에 사원입장의 매니저아이디는 매니저입장 에서는
사원아이디가 된다.
*/
select
...
/*
    using절에 사용된 참조컬럼의 경우 select 절에서 테이블의 별칭을 붙이면
    오히려 오류가 발생한다.
    using절에 사용된 컬럼은 양쪽의 테이블에 동시에 존재하는 컬럼이라는
    것을 전제로 작성되기 때문에 굳이 별칭을 사용할 이유가 없기때문이다.
    즉, using은 테이블의 별칭 및 on절을 제거하여 좀 더 심플하게 join쿼리문을
    작성할 수 있게 해준다.
*/
/*
퀴즈] 2005년에 입사한 사원들 중 California(STATE_PROVICE) /
    South San Francisco(CITY)에서 근무하는 사원들의 정보를 출력하시오.
    단, 표준방식과 using을 사용하여 작성하시오.
    
    출력결과] 사원번호, 이름, 성, 급여, 부서명, 국가코드, 국가명(COUNTRY_NAME)
        급여는 세자리마다 콤마를 표시한다.
    참고] '국가명'은 countries 테이블에 입력되어 있다.
*/
-- 1. 2005년에 입사한 사원
