...
*/
-- Oracle 21c 이상부터는 계정 생성전 해당명령을 실행해야 한다.
...
/*
tb_member 테이블에 새로운 레코드를 삽입한다.(DML 부분에서 학습예정)
...
/*
오라클 11g 에서는 계정을 생성한 후 connect, resource 를 (Roll)만 부여하면
테이블 생성 및 삽입까지 되지만 그 이후버전 테이블스페이스 관련 오류가 발생한다.
따라서 아래와 같이 테이블스페이스에 대한 권한도 부여해야 한다. 
...
create table tb_member_empty
    as select * from tb_member where 1 = 0;
